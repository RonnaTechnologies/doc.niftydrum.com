...
The sound board has roughly the same latency of 4.5ms, which means that, including a scan time of about 2 or 2.5ms for a trigger input, the total latency is about: 2.5 + 4.5 + 1 = 8ms. I added 1ms to account for UART transfer between the trigger board and the sound board, as well as audio codec latency.
